...
public class Mistral_driftStats extends BaseShipSystemScript {
    ...
    private final float HARD_FLUX_DISSIPATION_FRACTION = 0.5f;
    ...
    public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {
        ...
        //lets hard flux bleed off passively (normally it only clears while venting) for as
        //long as the system is charging, independent of shield state - excludes the player's
        //flagship specifically, autopilot or not; only applies to every other ship using it
        boolean isPlayerFlagship = ship != null && ship == Global.getCombatEngine().getPlayerShip();
        if (!isPlayerFlagship) {
            stats.getHardFluxDissipationFraction().modifyFlat(id, HARD_FLUX_DISSIPATION_FRACTION);
        }
