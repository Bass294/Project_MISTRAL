...
// ufhqiwrefhqwpeio by Bass294

public class mistral_himac_big extends BaseHullMod {
    ...
    static {
        BLOCKED_HULLMODS.add("armaa_himac");
    }
    ...
    private static class armaa_himacdata {
        //claude nonsense 1...
        // --- ADDED: timer for post-dash jitter/afterimage visuals ---
        float boostVisualTime = 0f;
        static final float BOOST_VISUAL_DURATION = 0.75f;
    ...
    public void createBoostParticles(ShipAPI ship, boolean assaultBoost) {
        ...
        if (!assaultBoost) {
            switch (ship.getHullSize()) {
                ...
                case FIGHTER:
                    Global.getSoundPlayer().playSound("mistral_drift", 1f, 0.6f * soundScale, ship.getLocation(), ZERO);
                    break;
                case FRIGATE:
                    Global.getSoundPlayer().playSound("mistral_drift", 1f, 0.8f * soundScale, ship.getLocation(), ZERO);
                    break;
                default:
                case DESTROYER:
                    Global.getSoundPlayer().playSound("mistral_drift", 1f, 1f * soundScale, ship.getLocation(), ZERO);
                    break;
                case CRUISER:
                    Global.getSoundPlayer().playSound("mistral_drift", 1f, 1f * soundScale, ship.getLocation(), ZERO);
                    break;
                case CAPITAL_SHIP:
                    Global.getSoundPlayer().playSound("mistral_drift", 1f, 1.2f * soundScale, ship.getLocation(), ZERO);
                    break;
            }
        }
    }
    ...
    public void boost(float angleDegrees, ShipAPI ship, boolean assaultBoost) {
        ...
        if (assaultBoost) {
            ...
            data.cooldown -= BOOST_COST * modifier;
        }
        if (MagicRender.screenCheck(0.1f, ship.getLocation())) {
    ...
    public void applyEffectsAfterShipCreation(ShipAPI ship, String id) {
        for (String tmp : BLOCKED_HULLMODS) {
            if (ship.getVariant().getHullMods().contains(tmp)) {
                MagicIncompatibleHullmods.removeHullmodWithWarning(ship.getVariant(), tmp, "mistral_himac_big");
            }
        }
    }
    ...
    public void advanceInCombat(ShipAPI ship, final float amount) {
        ...
        //claude nonsense 3....
        // --- ADDED: post-dash jitter and afterimage visuals ---
        if (data.boostVisualTime > 0f) {
            data.boostVisualTime -= amount;
            float effect = Math.max(0f, data.boostVisualTime / armaa_himacdata.BOOST_VISUAL_DURATION);
            ...
            if (Math.random() > 0.5f) {
                ship.addAfterimage(
                        ...
                        0, 0,
                        -ship.getVelocity().x * effect,
                        -ship.getVelocity().y * effect,
                        5f + 50f * effect,
                        0, 0,
                        0.6f + 0.4f * effect,
                        false, false, false
...
*/
    ...
    public boolean isApplicableToShip(ShipAPI ship) {
        return (ship.isFrigate() || ship.isDestroyer())
                && !ship.getVariant().hasHullMod("armaa_himac");
    }
    ...
    public String getUnapplicableReason(ShipAPI ship) {
        ...
        if (ship.getVariant().hasHullMod("armaa_himac")) {
            return "Incompatible with HIMAC Thrusters (Standard).";
        }
