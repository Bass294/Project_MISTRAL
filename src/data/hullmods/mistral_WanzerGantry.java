...
public class mistral_WanzerGantry extends BaseHullMod {
    private static final String GANTRY_ID = "mistral_wanzergantry";
    private final Color HL = Global.getSettings().getColor("hColor");
    private final String ID = "wanzer_gantry", TAG = "wanzer", TAG2 = "arbitrator";


    @Override
    public String getDescriptionParam(int index, HullSize hullSize) {
        if (index == 0) {
            return txt("hm_gantry_101b");
        } else if (index == 1) {
            return txt("hm_gantry_101c");
        } else if (index == 2) {
    ...
    // OLD VERSION OF GANTRIES NOT USED
    /*@Override
...
//                            10, Color.ORANGE, ship, 1, 1);
                    ...
                    switch (baseCount) {
                        ...
                        case 2:
                            ...
                            for (ShipAPI fighter : bay.getWing().getWingMembers()) {
                                fighter.getMutableStats().getBallisticWeaponDamageMult().modifyPercent(ID, 30f);
                                fighter.getMutableStats().getEnergyWeaponDamageMult().modifyPercent(ID, 30f);
                                fighter.getMutableStats().getMissileWeaponDamageMult().modifyPercent(ID, 30f);
    ...
    public void applyEffectsAfterShipCreation(ShipAPI ship, String id) {
        ...
        for (String w : ship.getVariant().getFittedWings()) {
            if (Global.getSettings().getFighterWingSpec(w).hasTag(TAG) || Global.getSettings().getFighterWingSpec(w).hasTag(TAG2)) {
    ...
    // changed because I added arbitrators
    ...
    private boolean noWanzer(ShipVariantAPI variant) {
        ...
        for (String w : variant.getFittedWings()) {
            if (Global.getSettings().getFighterWingSpec(w).hasTag(TAG)
                    || Global.getSettings().getFighterWingSpec(w).hasTag(TAG2)) {
                noWanzer = false;
                break;
            }
        }
